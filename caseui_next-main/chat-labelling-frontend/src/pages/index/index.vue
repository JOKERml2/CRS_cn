...
export default {
  ...
  methods: {
    ...
    async do () {
      ...
      console.log('re is :' + response.name)
      if (response === null) {
        return
      }
      this.touser = response.name
      // eslint-disable-next-line no-unused-vars
      let usrURL = 'http://localhost:9191/login?name=' + response.name + '&password=' + response.name + '&role=' + this.role + '&action=' + 'login'
      axios.post(
        usrURL
      ).then((json) => {
        this.$http.post('/checkLogin',
          'username=' + response.name + '&password=' + response.name,
